...
import { Search, Flame, MessageSquare, UserPlus, Bell, ChevronRight, Pin, X, Users, Plus, Clock, Loader2, Check } from 'lucide-react';
...
function Messages() {
  ...
  const [isLoadingNoteNotifications, setIsLoadingNoteNotifications] = useState(false);
  ...
  const messageCategories = [
    ...
    { id: 'notifications', label: t('messages.system') || '系统消息', icon: Bell, count: unreadCount },
  ];

  const tabs = [
    { id: 'recent', label: t('messages.recent') || '最近对话' },
    { id: 'groups', label: t('messages.groups') || '群聊' },
    { id: 'friends', label: t('messages.friends') || '好友' },
    { id: 'silent-rooms', label: t('messages.silent-rooms') || '房间' },
    { id: 'notifications', label: t('messages.notifications') || '通知' },
  ];
  ...
  const fetchAnnouncements = async () => {
    ...
    try {
      ...
      params.append('select', 'id,title,content,type,is_pinned,is_active,created_at,target_tags');
      ...
      if (Array.isArray(data)) {
        ...
        // 过滤公告：target_tags 为空/null 则全员可见，否则只显示匹配的
  ...
  const fetchGroupChats = async () => {
    ...
    try {
      // 查询所有 status=published 的 posts，前端过滤 __group_chat__ 标签
      const params = new URLSearchParams();
      ...
      if (Array.isArray(data)) {
        // 前端过滤：只显示包含 __group_chat__ 标签 且 当前用户是成员的群聊
        const memberTag = currentUserId ? `member_${currentUserId}` : '';
        const groupChats = data.filter((post: GroupChat) => {
          if (!post.tags?.includes('__group_chat__')) return false;
          // 只显示用户已加入的群聊（tags中包含 member_{userId}）
  ...
  // 获取当前用户创建的待审核群聊
  const fetchPendingGroupChats = async () => {
    if (!currentUserId) return;
    
    try {
      const params = new URLSearchParams();
      params.append('select', 'id,title,content,tags,user_id,status,created_at');
      params.append('tags', 'cs.__group_chat__');
      params.append('user_id', `eq.${currentUserId}`);
      params.append('or', '(status.eq.pending,status.eq.rejected)');
      ...
      if (Array.isArray(data)) {
        setPendingGroupChats(data);
      }
    } catch (error) {
      console.error('Error fetching pending group chats:', error);
    }
  };
  ...
  const sendFriendRequest = async () => {
    ...
    try {
      const followRecord = {
        ...
        // 添加打招呼消息（如果 follows 表有 message 列会存储，没有则忽略）
  ...
  const loadFriendsList = async () => {
    ...
    try {
      // 查询我关注的人（follower_id=我, status=active）
      const params = new URLSearchParams();
  ...
  const markAsRead = (announcement: Announcement) => {
    try {
      localStorage.setItem('last_read_announcement_id', announcement.id);
    } catch (e) {
      console.error('Failed to save announcement read status:', e);
    }
  ...
  // 获取笔记申请通知
  const fetchNoteNotifications = async () => {
    const userId = localStorage.getItem('user_id');
    if (!userId) return;
    setIsLoadingNoteNotifications(true);
    try {
      const params = new URLSearchParams();
      ...
      setIsLoadingNoteNotifications(false);
    }
  };

  // 标记笔记通知为已读
  const markNoteNotificationAsRead = async (notification: any) => {
  ...
  // 全部标记笔记通知已读
  const markAllNoteNotificationsAsRead = async () => {
    const userId = localStorage.getItem('user_id');
    if (!userId) return;
    const unreadIds = noteNotifications.filter(n => !n.is_read).map(n => n.id);
    if (unreadIds.length === 0) return;
    try {
      await fetch(`/sb-api/rest/v1/notifications?user_id=eq.${userId}&is_read=eq.false`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'apikey': SERVICE_ROLE_KEY,
      ...
      setNoteNotifications(prev => prev.map(n => ({ ...n, is_read: true })));
    } catch (error) {
    ...
    if (announcements.length === 0) {
      return (
        <div className="text-center py-12">
          ...
          <p style={{ color: 'var(--text-secondary)' }}>{t('messages.noAnnouncements') || '暂无公告'}</p>
        </div>
      );
    }
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  {announcement.is_pinned && (
                    <Pin className="w-3.5 h-3.5 flex-shrink-0" style={{ color: PRIMARY_COLOR }} />
                  )}
                  <span 
                    className="font-medium text-sm truncate"
                    style={{ color: 'var(--text-color)' }}
  ...
  // 渲染笔记通知Tab
  const renderNoteNotificationsTab = () => {
    const unreadCount = noteNotifications.filter(n => !n.is_read).length;

    if (isLoadingNoteNotifications) {
      return (
        <div className="flex items-center justify-center py-12">
          <div className="w-6 h-6 border-2 border-[#E11D48] border-t-transparent rounded-full animate-spin" />
        </div>
      ...
      <div className="space-y-2">
        ...
        {unreadCount > 0 && (
          <div className="flex justify-end mb-2">
            <button
              onClick={markAllNoteNotificationsAsRead}
              className="text-xs px-3 py-1.5 rounded-lg transition-colors"
              style={{ color: PRIMARY_COLOR, backgroundColor: `${PRIMARY_COLOR}10` }}
            >
              全部已读
            </button>
          </div>
        ...
        {noteNotifications.length === 0 ? (
          <div className="text-center py-12">
            <Bell className="w-12 h-12 mx-auto mb-3 opacity-30" style={{ color: 'var(--text-secondary)' }} />
            <p style={{ color: 'var(--text-secondary)' }}>暂无笔记通知</p>
          </div>
        ) : (
          noteNotifications.map((notification) => (
            <button
              key={notification.id}
              onClick={() => markNoteNotificationAsRead(notification)}
              className="w-full text-left p-4 rounded-xl transition-colors hover:opacity-80"
              style={{ backgroundColor: notification.is_read ? 'var(--bg-secondary)' : `${PRIMARY_COLOR}08` }}
            >
              <div className="flex items-start gap-3">
                {/* 未读指示器 */}
                ...
                <div
                  ...
                  style={{
                    backgroundColor: notification.type === 'note_request_approved'
                      ? '#10b98120'
                      : '#ef444420',
                  }}
                >
                  {notification.type === 'note_request_approved' ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    >
                      {notification.type === 'note_request_approved' ? '已通过' : '已拒绝'}
                    </span>
                  </div>
  ...
  const renderEmptyState = () => (
    <div className="text-center py-12">
      <p style={{ color: 'var(--text-secondary)' }}>{t('messages.empty')}</p>
    </div>
  );
    ...
    >
          ...
          {tabs.map((tab) => (
            ...
            >
              ...
              {tab.badge && tab.badge > 0 && (
                <span
                  className="ml-1 w-4 h-4 rounded-full text-[10px] flex items-center justify-center text-white"
      ...
      <div className="px-4">
        {activeTab === 'groups' ? renderGroupsTab() :
         ...
         activeTab === 'silent-rooms' ? (
           <RoomList 
             onClose={() => setActiveTab('recent')}
             onCreateRoom={() => setShowCreateRoom(true)}
           />
         ) :
         activeTab === 'notifications' ? renderNotificationsTab() : renderRecentTab()}
      </div>
      ...
      {greetingModal && (
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                <UserPlus className="w-5 h-5" style={{ color: PRIMARY_COLOR }} />
                <h3 className="font-bold text-gray-900">发送好友请求</h3>
              </div>
              <button 
                onClick={() => setGreetingModal(null)}
                className="p-1.5 rounded-full hover:bg-gray-100 transition-colors"
