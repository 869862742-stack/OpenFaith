      ...
      <div className="fixed inset-0 flex flex-col items-center justify-center bg-[#1a1a2e]">
        ...
        <div className="w-72 flex flex-col gap-3">
          <input
            ...
            placeholder="访问密码"
            className="w-full h-11 px-4 rounded-xl bg-white/10 text-white text-sm border border-white/20 focus:border-[#2563EB] focus:outline-none placeholder-gray-500"
          />
          <button
            onClick={() => {
              if (accessInput === ACCESS_PASSWORD) {
            ...
            className="w-full h-11 bg-[#2563EB] text-white font-medium rounded-xl hover:bg-[#c41a3f] transition-colors"
          >
            进入
          </button>
    ...
    <ErrorBoundary>
      <React.Suspense fallback={<PageLoader />}>
        <Routes>
          ...
          {/* 404 处理 */}
          <Route path="*" element={<div style={{padding:'40px',textAlign:'center'}}><h2>{t('errors.notFound')}</h2><p>路径: {window.location.hash}</p><button onClick={()=>window.location.hash='/'} style={{marginTop:'20px',padding:'10px 20px',background:'#2563EB',color:'white',border:'none',borderRadius:'5px',cursor:'pointer'}}>{t('common.backHome')}</button></div>} />
        </Routes>
      </React.Suspense>
    </ErrorBoundary>
  );
